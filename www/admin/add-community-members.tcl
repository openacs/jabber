ad_page_contract {
    ...
    add selected community members
    @author Luis Mosteiro Fernandez
    @creation-date 2002-12-04
    @param user_id to which user
    @param community_members array of user_ids, which should be added
...
for {set i 1} { $i <= $rows} {incr i} {
    ...
    if {[info exists community_members($i)]} {
	set add_user_id $community_members($i)
