...
<table width='400' border='0' bgcolor='#666666' cellpadding='1' cellspacing='0'>
 <tr>
  <td bgcolor='#333333'>
   <table width='400' border='0' bgcolor='#FFFFFF' cellpadding='0'>
    ...
    <tr bgcolor='#CCCCCC'> 
     <td colspan='5'>This icons display your online State.<br> Log in to the @system_name@ Jabber by clicking on the yellow Links below. 
       <if @jabber_start@ eq offline>
        <ul> 
	<li><a href='/jabber/jabber-login?return_url=@return_url@'><font color='yellow'>Start Jabber Applet</font></a></li>
 ...
 <if @member_is_online_p@ eq 1>
 <table width='700' border='0' bgcolor='#666666' cellpadding='1' cellspacing='0'>
  <tr>
   <td>
    <table width='700' border='0' bgcolor='#FFFFFF'>
     <tr bgcolor='#999999'> 
      <td colspan='8'><b><font color='#FFFFFF'>Class Members online</font></b></td>
     </tr>
     <multiple name="member_status">
      <td width='20%'>@member_status.first_names@ @member_status.last_name@</td>
      <if @member_status.member_is_friend@ eq 0>	
       <td width='20%'><a href='buddy-add?friend_id=@member_status.member_user_id@&return_url=@return_url@'>Add Member to Friends list</a></td>
      </if>
	<group column="member_user_id">
	<if @member_status.status@ not eq offline>
	 <if @member_status.service@ eq aim>
	  <td width='10%'><div align='center'><a href='aim?screen=@member_status.im_screen_name@'>
         </if>
         <else>
          <td width='10%'><div align='center'><a href='message?screen=@member_status.create_jid@&screen_id=@member_status.member_screen_id@'>
         </else>
         <img src='image/@member_status.service@_@member_status.status@.gif' width='18' height='25'></a>
         </div>
      	 </td>
	 </if>     
	 </group>
	<if @member_status.chat@  not eq  "0 0" >
      	 <td width='10%'><div align='center'><a href='@member_status.chat@'>Chat </a> 
          </div>
         </td>
	</if>
     </tr>
   ...
   <td>
	...
	<if @friend_status.status@ not eq offline>
	 <if @friend_status.service@ eq aim>
	  <td width='10%'><div align='center'><a href='aim?screen=@friend_status.im_screen_name@'>
         </if>
         <else>
	...
	<if @friend_status.chat@  not eq  "0 0" >
